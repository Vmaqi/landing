...
// filter array
var anubisFilterArray		= [];
var anubisFilterCondition	= 'and';


(function($){
  "use strict";
	
	var oogPreloader 	= false;
  	var oogTime 		= new Date();
	var oogCounter		= 0;
	...
	var ooganubis = {
		...
		init: function(){
			ooganubis.ready();
			ooganubis.menuCenter();
			ooganubis.imgToSVG();
			ooganubis.BgImg();
			ooganubis.totopFixer();
			ooganubis.slider();
			ooganubis.headerFixer();
			ooganubis.fn_cs_counter();
			ooganubis.video();
			ooganubis.collection();
			ooganubis.contactForm();
			ooganubis.totopScroll();
			ooganubis.seachSomething();
			ooganubis.walletAndLeftNavOpener();
			ooganubis.resizeWalletAndLeftNav();
			ooganubis.headerAnchor();
			ooganubis.isotope();
			ooganubis.qnt();
			ooganubis.countdown();
			ooganubis.hold();
			ooganubis.filterItems();
			ooganubis.applyFilter();
			ooganubis.navSubMenu();
			ooganubis.roadmapSwiper();
			ooganubis.floww();
			ooganubis.accordion();
			ooganubis.hero4Swiper();
			ooganubis.rippleEffect();
			ooganubis.fullSlider();
		},
		...
		floww: function(){
			ooganubis.BgImg();
			$('.oog_cards_gallery').each(function(){
				var element 		= $(this),
					ul				= element.find('ul'),
				...
				ooganubis.floww_change_slide(1,element);

				// item click function
				children.on('click',function(){
					var el 			= $(this);
					var index		= el.index() + 1;
					ooganubis.floww_change_slide(index,element);
				});
				ooganubis.floww_start_autoplay(ul,element);
			});
		},
		...
		floww_change_slide: function(index, element){
			...
			if(!el.hasClass('current')){
				...
				ooganubis.floww_calc_call(element);
			}
		},
		...
		roadmapSwiper: function(){
			...
			section.each(function(){
				...
				ooganubis.roadmapStep(mySwiper,step,widthParts);
				
				mySwiper.on('slideChange', function () {
					ooganubis.roadmapStep(this,step,widthParts);
				});
			});	
		},
		...
		navSubMenu: function(){
			$('.anubis_fn_leftnav .nav_holder a').off().on('click',function(){
				var e = $(this);
				var s = e.siblings('.sub-menu');
				var svg = $('.anubis_fn_leftnav .nav_holder .icon').html();
				if(s.length){
					...
					$('.anubis_fn_leftnav .nav_holder > ul').css({transform: 'translateX(-'+(100*oogCounter)+'%)'});
					ooganubis.previousItems();
					return false;
				}
			});
		},
		...
		previousItems: function(){
			$('.anubis_fn_leftnav .nav_holder .prev').off().on('click',function(){
				oogCounter--;
				$('.anubis_fn_leftnav .nav_holder > ul').css({transform: 'translateX(-'+(100*oogCounter)+'%)'});
				return false;
			});	
		...
		applyFilter: function(){
			...
			ooganubis.isotopeCollection();
			
			// left filter on click function
			$('.anubis_fn_filters .checkbox').off().on('click',function(){
				
					// our clicked filter
				var element 	= $(this),
					
					// collection wrapper
					parent		= element.closest('.anubis_fn_collection'),
					
					// filter result box
					resultBox	= parent.find('.anubis_fn_result_box'),
				...
				// if clicked item has already clicked and clicked second time
					...
					var index = anubisFilterArray.indexOf(id);
					if(index !== -1){
						anubisFilterArray.splice(index, 1);
					}
				}
				
				
				ooganubis.recallGridAfterFiltering();
		...
		recallGridAfterFiltering: function(clear){
			...
			if(anubisFilterArray.length === 0){
				filters = '*';
			}
			// if there has one or more selected filters
			else{
				if(anubisFilterCondition === 'or'){
					$.each(anubisFilterArray, function(index,value){
						filters += '.id'+value+',';
					});
		...
		removeFilter: function(){
			$('.anubis_fn_result_box .result_item a').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.anubis_fn_collection'),
					resultItem	= e.closest('.result_item'),
					resultBox	= parent.find('.anubis_fn_result_box'),
					id 			= resultItem.data('id'),
					filterCount = resultBox.find('.filter_count span');
				resultItem.remove();
				parent.find('.anubis_fn_filters .checkbox[data-id="'+id+'"]').removeClass('selected').find('input[type="checkbox"]').prop('checked','');
				filterCount.text(parseInt(filterCount.text())-1);
				if(resultBox.find('.result_item').length === 0){
					resultBox.find('.clear_all').remove();
					resultBox.find('.condition_trigger').remove();
			...
			$('.anubis_fn_result_box .clear_all').off().on('click',function(){
				var e 			= $(this),
					parent		= e.closest('.anubis_fn_collection'),
					filterBox	= parent.find('.anubis_fn_filters'),
					resultBox	= parent.find('.anubis_fn_result_box'),
					filterCount = resultBox.find('.filter_count span');
				filterCount.text(0);
				...
				ooganubis.recallGridAfterFiltering('clear');
				return false;
			});
			
			$('.anubis_fn_result_box .condition_trigger').off().on('click',function(){
				var e 			= $(this);
		...
		filterItems: function(){
			$('.anubis_fn_filters .filter_item.opened').each(function(){
				var e = $(this);
		...
		hold: function(){
		var holdable					= $('#social,.anubis_fn_search');
		...
		countdown: function(){
			$('.anubis_fn_countdown').each(function(){
				var e = $(this),
					t = e.data('type');
				if(t === 'due_date'){
				...
				var p = e.parent();
				if(e.hasClass('boxed')){
					e.after('<div class="anubis_fn_boxed_countdown"><ul><li class="days"><div class="item"><div class="count"><h3 class="fn__gradient_title"></h3></div><span>Days</span></div></li><li class="hours"><div class="item"><div class="count"><h3 class="fn__gradient_title"></h3></div><span>Hours</span></div></li><li class="minutes"><div class="item"><div class="count"><h3 class="fn__gradient_title"></h3></div><span>Minutes</span></div></li><li class="seconds"><div class="item"><div class="count"><h3 class="fn__gradient_title"></h3></div><span>Seconds</span></div></li></ul></div>');
					e.remove();
				}
				if(t === 'due_date'){
		...
		qnt: function(){
			...
			$('.qnt .increase').off().on('click',function(){
				var e = $(this),
					s = e.closest('.qnt').find('.summ');
				var sVal = parseInt(s.text());
				if (sVal < 2) { // Check if the current value is less than 2 before increasing
					sVal++;
					s.text(sVal);
					var price = sVal * s.data('price');
					s.closest('.mint_list').find('.total_price').html(price);
		...
		resizeWalletAndLeftNav: function(){
			var walletHeight = $('.anubis_fn_walletbox').height();
			$('.anubis_fn_walletbox .walletbox').css({minHeight: walletHeight});
			
			
			var leftnavHeight = $('.anubis_fn_leftnav').height();
			$('.anubis_fn_leftnav .navbox').css({minHeight: leftnavHeight});
		},
		
		ready: function(){
			$('.anubis_fn_walletbox, .anubis_fn_wallet_closer, .anubis_fn_leftnav, .anubis_fn_leftnav_closer').removeClass('ready');
		},
		
		walletAndLeftNavOpener: function(){
			var walletbox 	= $('.anubis_fn_walletbox');
			var closer		= $('.anubis_fn_wallet_closer,.anubis_fn_walletbox .fn__closer');
			...
			var leftNav 	= $('.anubis_fn_leftnav');
			var closer2		= $('.anubis_fn_leftnav_closer,.anubis_fn_leftnav .fn__closer');
			$('.header .trigger,.anubis_fn_mobnav .social_trigger .trigger').on('click',function(){
			...
			var mobOpener 	= $('.anubis_fn_mobnav .mob_mid .trigger');
			var mobDD		= $('.anubis_fn_mobnav .mob_bot');
		...
		preloader: function(){
			if(oogPreloader){return false;}oogPreloader = true;
  			var date2 			= new Date();
			var difference		= date2-oogTime;
			var waitTime 		= 2000;
			...
			setTimeout(function(){
				$('.anubis_fn_preloader').addClass('ready');
			},waitTime);
		},
		
		seachSomething: function(){
			var searchOpener 	= $('.anubis_fn_search');
			var searchbox 		= $('.anubis_fn_searchbox');
			var searchCloser	= $('.anubis_fn_search_closer');
			...
			button.on('click',function(){
				var string = $('.anubis_fn_searchbox input').val();
				window.find(string);
				searchbox.removeClass('active');
				searchCloser.removeClass('active');
				return false;
		...
		totopScroll: function(){
			var minSpeed 		= 500;
			var maxSpeed		= 1500;
			$(".anubis_fn_totop").off().on('click', function(e) {
				e.preventDefault();
		...
		slider: function(){
			$('.fn_cs_slider').each(function(){
				...
				var myInterval 		= setInterval(function(){
					activeIndex++;
					activeIndex 	= ooganubis.sliderDo(sliderTop,sliderBottom,activeIndex);
				},speed);
		...
		totopFixer: function(){
			var w = $('.anubis_fn_totop .totop_inner').width();	
			$('.anubis_fn_totop').css({height: w + 'px'});
		},
